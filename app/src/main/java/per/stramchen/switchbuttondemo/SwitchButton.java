package per.stramchen.switchbuttondemo;
...
/**
 * 自定义SwitchButton.神还原IOS开关按钮。
 * Style加入以下配置文件，就可以使用了
 ...
 接下来引入自定义布局
 <com.mxchip.ap25.openabase.ui.widget.SwitchButton
 android:id="@+id/btn_appointment_switch"
 ...
 app:sb_background="@color/switch_bg"
 app:sb_checked_color="#00CB90"
...
public class SwitchButton extends View implements Checkable {
    ...
    public void setChecked(boolean checked) {
        ...
        //如果动画正在进行，check的值其实是动画将要到达的状态，那么return
        if(valueAnimator.isRunning() && checked == !isChecked()){
            return;
        }
    ...
    private void toggle(boolean animate, boolean broadcast) {
        if(!isEnabled()){return;}
        shouldEventBroadcast = broadcast;
     ...
     *
     */
    private boolean shouldEventBroadcast = true;
